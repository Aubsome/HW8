...
class MainWindow(qtw.QWidget, Ui_Form):
    ...
    def SelectQualityOrTHigh(self):
        if self.rdo_Quality.isChecked():
            self.RC.Model.selectQualityOrTHigh('Quality')
        else:
            self.RC.Model.selectQualityOrTHigh('THigh')

    def SetPlotVariables(self):
        x_axis = self.cmb_XAxis.currentText()
        y_axis = self.cmb_YAxis.currentText()
        log_x = self.chk_logX.isChecked()
        log_y = self.chk_logY.isChecked()
        self.RC.View.updatePlot(x_axis, y_axis, log_x, log_y)

    def SetUnits(self):
        if self.rb_SI.isChecked():
            self.RC.View.updateUnits('SI')
        else:
